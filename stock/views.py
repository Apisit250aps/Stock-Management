...
def getProvince(request):
    ...
    return Response({'status': True, 'message': msg, 'data': data['province']})
...
def getDistrict(request):
    ...
    return Response({'status': True, 'message': msg, 'data': data['district']})
...
def getTambon(request):
    ...
    return Response({'status': True, 'message': msg, 'data': data['tambon']})
...
def createInputData(request):
    ...
    try :
        ...
        models.InputInvoice.objects.filter(invoice_id=invoice.invoice_id).update(invoice_no=f"I{shop.shop_code}{invoice.invoice_id}")
        ...
        for product in products:
            ...
            models.ProductData.objects.filter(product_id=_product.product_id).update(product_code=f"P{_product.product_id}")
                
            print(f"No. {models.InputInvoice.objects.get(invoice_id=invoice.invoice_id).invoice_no}")
                    
            models.InputData.objects.create(
                invoice = invoice,
                invoice_no = models.InputInvoice.objects.get(invoice_id=invoice.invoice_id).invoice_no,
                product = _product,
            ...
            models.TempInputDB.objects.create(
                ...
                invoice_no = models.InputInvoice.objects.get(invoice_id=invoice.invoice_id).invoice_no,
                product = models.ProductData.objects.get(product_id=_product.product_id).product_code,
                quantity = product_unit,
                discount=product_disc,
                unit_price=product_cost
...
def createShop(request):
    ...
    try :
        ...
        if _user :
            ...
            models.ShopData.objects.filter(shop_id=shop.shop_id).update(shop_code=f"S{shop.shop_id}")
...
def deleteShop(request):
    ...
    User.objects.filter(username=models.ShopData.objects.get(shop_id=id).user).delete()
